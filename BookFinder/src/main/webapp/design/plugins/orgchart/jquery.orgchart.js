(function($) {
	...
	$.fn.orgChart.defaults = {
		...
		onDeleteNode : null,
		onClickNode : null,
		addNodeText : '<button class="team__button--add">추가</button>',
		removeNodeText : '<button class="team__button--remove">삭제</button>'
	};
	...
	function OrgChart($container, opts) {
		...
		this.startEdit = function(id) {
			var oldName = nodes[id].data.name;
			var inputElement = $('<input class="org-input" type="text" value="'
					+ nodes[id].data.name + '"/>');
			$container.find('div[node-id=' + id + '] h2').replaceWith(
					inputElement);
			var commitChange = function() {
				...
				if (nodes[id].data.name == "") {
					...
					var current = nodes[id].data;
					var parent = nodes[current.parent].data;
					...
					if (!Admin.prototype.startOrgChartCommit(data)) {
						self.deleteNode(id);
					}
				}
			}
	...
	function Node(data) {
		...
		this.render = function(opts) {
			...
			mainTable += "<tr><td colspan='" + nodeColspan + "'>"
					+ self.formatNode(opts) + "</td></tr>";
			...
			if (childLength > 0) {
				...
				mainTable += "<tr class='lines'><td colspan='" + childLength
						* 2 + "'>" + downLineTable + '</td></tr>';

				var linesCols = '';
				for (var i = 0; i < childLength; i++) {
					if (childLength == 1) {
						linesCols += "<td class='line left half'></td>"; // keep
						// vertical
						// lines
						// aligned
						// if
						...
						linesCols += "<td class='line left'></td>"; // the first
						// cell
						// doesn't
						// have a
						// line in
				...
				for ( var i in self.children) {
					mainTable += "<td colspan='2'>"
							+ self.children[i].render(opts) + "</td>";
				}
				mainTable += "</tr>";
			}
			mainTable += '</table>';
		...
		this.formatNode = function(opts) {
			...
			if (opts.showControls) {
				if (this.data.parent > 0) {
					var buttonsHtml = "<div class='org-add-button'>"
							+ opts.addNodeText
							+ "</div><div class='org-del-button'>"
							+ opts.removeNodeText + "</div>";
				} else {
					var buttonsHtml = "<div class='org-add-button' style='left: 28px;'>"
							+ opts.addNodeText + "</div>";
				}
			} else {
				buttonsHtml = '';
			}
			return "<div class='node' node-id='" + this.data.id + "'>"
					+ nameString + descString + buttonsHtml + "</div>";
		}
	}
